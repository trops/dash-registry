...
const GITHUB_USER = /^[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?$/;
const WIDGET_ID = /^[a-zA-Z0-9-]+\.[a-zA-Z0-9-]+\.[A-Z][a-zA-Z0-9]+$/;
...
function validateManifest(manifest, scopeDirName, pkgDirName, filePath) {
  ...
  // githubUser (new) or scope (legacy) — at least one required
  const identityField = manifest.githubUser || manifest.scope;
  if (!identityField) {
    err('"githubUser" (or legacy "scope") is required');
  } else if (typeof identityField !== "string") {
    err('"githubUser"/"scope" must be a string');
  } else {
    // githubUser uses GitHub username format; scope uses kebab-case
    if (manifest.githubUser) {
      if (!GITHUB_USER.test(manifest.githubUser)) {
        err(
          `"githubUser" must be a valid GitHub username (got "${manifest.githubUser}")`,
        );
      }
    } else {
    ...
    if (identityField !== scopeDirName) {
      err(
        `"githubUser"/"scope" must match scope directory name (got "${identityField}", directory is "${scopeDirName}")`,
      );
    }
  }
  ...
  if (!manifest.name) {
    ...
    if (typeof manifest.name !== "string") {
      ...
      if (manifest.name.length < 2 || manifest.name.length > 50) {
        err(
          '"name" must be 2–50 characters (got ' + manifest.name.length + ")",
        );
      }
      ...
      if (manifest.name !== pkgDirName) {
        err(
          `"name" must match directory name (got "${manifest.name}", directory is "${pkgDirName}")`,
        );
      }
    }
  }
  ...
  if (!manifest.displayName) {
    ...
    err(
      '"displayName" must be at most 100 characters (got ' +
        manifest.displayName.length +
        ")",
    );
  }
  ...
  if (!manifest.downloadUrl) {
    ...
    !isValidUrl(manifest.downloadUrl.replace(/\{[^}]+\}/g, "placeholder"))
  ) {
    err(
      `"downloadUrl" must be a safe HTTPS URL (got "${manifest.downloadUrl}")`,
    );
  }
  ...
  if (!manifest.widgets) {
    ...
    manifest.widgets.forEach((widget, i) => {
      ...
      if (!widget.name) {
        ...
        err(
          `${prefix}.name "${widget.name}" is a duplicate within this package`,
        );
      } else {
        widgetNames.add(widget.name);
      }

      // widget.id (optional but validated if present)
      if (widget.id !== undefined) {
        if (typeof widget.id !== "string" || !widget.id.trim()) {
          err(`${prefix}.id must be a non-empty string`);
        } else if (!WIDGET_ID.test(widget.id)) {
          err(
            `${prefix}.id must match "{user}.{package}.{WidgetName}" format (got "${widget.id}")`,
          );
        } else if (identityField && manifest.name && widget.name) {
          // Verify id components match manifest fields
          const expectedId = `${identityField}.${manifest.name}.${widget.name}`;
        ...
        if (!Array.isArray(widget.providers)) {
          ...
          widget.providers.forEach((p, j) => {
            if (!p.type || typeof p.type !== "string") {
              warn(
                `${prefix}.providers[${j}].type should be a non-empty string`,
              );
            }
  ...
  if (manifest.category !== undefined) {
    if (!VALID_CATEGORIES.includes(manifest.category)) {
      warn(
        `"category" not recognized (got "${manifest.category}", expected one of: ${VALID_CATEGORIES.join(", ")})`,
      );
    }
  }
  ...
  if (manifest.tags !== undefined) {
    if (!Array.isArray(manifest.tags)) {
      ...
      if (manifest.tags.length > 10) {
        warn(
          '"tags" should have at most 10 items (got ' +
  ...
  if (manifest.repository !== undefined) {
    if (typeof manifest.repository !== "string") {
      ...
      err(
        `"repository" must be a safe HTTPS URL (got "${manifest.repository}")`,
      );
    }
  }
  ...
  if (manifest.publishedAt !== undefined) {
    if (
      ...
      !ISO_8601.test(manifest.publishedAt)
    ) {
      warn(
        `"publishedAt" should be a valid ISO 8601 date (got "${manifest.publishedAt}")`,
      );
    }
  }
  ...
  if (
    ...
    typeof manifest.deprecated !== "boolean"
  ) {
  ...
  if (
    manifest.deprecatedMessage !== undefined &&
    typeof manifest.deprecatedMessage !== "string"
  ) {
...
function main() {
  ...
  for (const scopeDir of scopeDirs) {
    ...
    for (const pkgDir of pkgDirs) {
      ...
      const identity = manifest.githubUser || manifest.scope;
      if (identity && manifest.name) {
        const scopedName = `${identity}/${manifest.name}`;
        if (allScopedNames.has(scopedName)) {
          errors.push(
            `ERROR: ${relPath}: duplicate scoped package name "${scopedName}"`,
          );
        } else {
